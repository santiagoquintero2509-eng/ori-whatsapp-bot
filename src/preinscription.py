...
def update_confirmed_stand(query, stand, representative="", status="stand confirmado", confirmed_by="Ori admin"):
    payload = {
...
def log_conversation_event(event):
    payload = {
    ...
    url = webhook_url()
    if not url:
        queue_conversation_event(payload)
    ...
    try:
        ...
        if not parsed.get("ok"):
            raise RuntimeError(parsed.get("error") or body or "Apps Script no confirmo el historial.")
        return parsed
...
def queue_conversation_event(payload):
    try:
        CONVERSATION_EVENT_QUEUE_PATH.parent.mkdir(parents=True, exist_ok=True)
        with CONVERSATION_EVENT_QUEUE_PATH.open("a", encoding="utf-8") as file:
            file.write(json.dumps(payload, ensure_ascii=False) + "\n")
    except OSError as error:
        print(f"No se pudo guardar cola de historial de conversaciones: {error}", flush=True)
