...
def parse_admin_action(message, text):
    ...
    form_lookup_match = re.search(
        r"\b(?:busca|buscar|consulta|consultar|revisa|revisar|ya\s+(?:lleno|envio|diligencio)|esta\s+preinscrit[oa])\s+(?:a\s+|si\s+)?(.+?)(?:\s+(?:en\s+)?(?:el\s+)?formulario|$)",
        text,
    )
    if form_lookup_match and has_any(text, ["formulario", "preinscripcion", "preinscrito", "preinscrita", "lleno", "envio", "diligencio", "busca", "consulta", "revisa"]):
        query = clean_admin_query(form_lookup_match.group(1))
        if query and normalize(query) not in {"el", "la", "si", "formulario"}:
            return {"type": "form_lookup", "query": query}

    direct_form_match = re.search(r"\b[a-z0-9 ]{4,}\s+(?:ya\s+)?(?:lleno|envio|diligencio|esta\s+en)\s+(?:el\s+)?formulario\b", text)
    if direct_form_match:
        query = clean_admin_query(re.split(r"\b(?:ya\s+)?(?:lleno|envio|diligencio|esta\s+en)\b", text, maxsplit=1)[0])
        return {"type": "form_lookup", "query": query}
